...
from ..utils import ExplicitEnum, ModelOutput, is_accelerate_available, logging, diffusion_utils
...
from .configuration_utils import GenerationConfig, ContrastiveDecodingConfig
...
class GenerationMixin:
    ...
    def generate(
        ...
        **kwargs,
        ...
        # NOTE: if contrastive decoding config is provided, we are doing contrastive decoding
        contrastive_decoding = contrastive_decoding_config is not None
        # obtain input embeddings for contrastive decoding
        ...
        if generation_mode == GenerationMode.GREEDY_SEARCH:
            ...
            if contrastive_decoding:
                if contrastive_decoding_config.strategy == "VCD":
                    return self.greedy_search_with_visual_contrastive_decoding(
                        input_ids,
                        logits_processor=prepared_logits_processor,
                        stopping_criteria=prepared_stopping_criteria,
                        pad_token_id=generation_config.pad_token_id,
                ...
                elif contrastive_decoding_config.strategy == "SID":
                    return self.greedy_search_with_self_introspective_decoding(
                            input_ids,
                            logits_processor=prepared_logits_processor,
                            stopping_criteria=prepared_stopping_criteria,
                            pad_token_id=generation_config.pad_token_id,
                    ...
                    raise ValueError(f"Contrastive decoding strategy {contrastive_decoding_config.strategy} not supported")
            ...
            if contrastive_decoding:
                if contrastive_decoding_config.strategy == "VCD":
                    return self.sample_with_visual_contrastive_decoding(
                        input_ids,
                        logits_processor=prepared_logits_processor,
                    ...
                    return self.sample_with_self_introspective_decoding(
                        input_ids,
                        logits_processor=prepared_logits_processor,
                        logits_warper=logits_warper,
                        stopping_criteria=prepared_stopping_criteria,
    ...
    def greedy_search_with_visual_contrastive_decoding(
        self,
        input_ids: torch.LongTensor,
        logits_processor: Optional[LogitsProcessorList] = None,
        stopping_criteria: Optional[StoppingCriteriaList] = None,
        ...
        streamer: Optional["BaseStreamer"] = None,
        contrastive_decoding_config: Optional[ContrastiveDecodingConfig] = None,
        cd_inputs_embeds: Optional[torch.Tensor] = None,
        **model_kwargs,
    ) -> Union[GenerateNonBeamOutput, torch.LongTensor]:
        r"""
        Generates sequences of token ids for models with a language modeling head using **greedy decoding** and can be
        ...
        Parameters:
            ...
            streamer (`BaseStreamer`, *optional*):
                Streamer object that will be used to stream the generated sequences. Generated tokens are passed
                through `streamer.put(token_ids)` and the streamer is responsible for any further processing.
            contrastive_decoding_config (`ContrastiveDecodingConfig`, *optional*):
                An instance of [`ContrastiveDecodingConfig`]. Configuration object for contrastive decoding.
            model_kwargs:
                Additional model specific keyword arguments will be forwarded to the `forward` function of the model.
                If model is an encoder-decoder model the kwargs should include `encoder_outputs`.
        ...
        # VCD: Get visual contrastive decoding parameters
        alpha = contrastive_decoding_config.alpha
        beta = contrastive_decoding_config.beta

        # VCD: copy model_kwargs for visual contrastive decoding.
        vcd_model_kwargs = model_kwargs.copy()
        # VCD: change input_embeds to cd_inputs_embeds
        vcd_model_kwargs["inputs_embeds"] = cd_inputs_embeds.clone()
        # VCD: add random Gaussian noise to original images
        # diffusion = diffusion_utils.GaussianDiffusion()
        # images = vcd_model_kwargs["images"]
        # vcd_model_kwargs["images"] = diffusion.forward(images, noise_step)
        while True:
            if synced_gpus:
                # Under synced_gpus the `forward` call must continue until all gpus complete their sequence.
                # The following logic allows an early break if all peers finished generating their sequence
            ...
            # VCD: prepare model inputs
            vcd_model_inputs = self.prepare_inputs_for_generation(input_ids, **vcd_model_kwargs)

            # VCD: forward pass to get next token
            vcd_outputs = self(
                **vcd_model_inputs,
                return_dict=True,
                output_attentions=output_attentions,
            ...
            vcd_next_token_logits = vcd_outputs.logits[:, -1, :]

            # KEY PART OF VISUAL CONTRASTIVE DECODING
            # (1) compute adaptive plausibility constraint
            cutoff = torch.log(torch.tensor(beta)).to(input_ids.device) + torch.max(next_token_logits, dim=-1, keepdim=True).values
            # (2) contrast two logits
            diffs = (1 + alpha) * next_token_logits - alpha * vcd_next_token_logits
            # (3) apply adaptive plausibility constraint
            next_token_logits = torch.masked_fill(diffs, next_token_logits < cutoff, float("-inf"))
            ...
            # VCD: update generated ids, model inputs, and length for next step
            vcd_model_kwargs = self._update_model_kwargs_for_generation(
                vcd_outputs, vcd_model_kwargs, is_encoder_decoder=self.config.is_encoder_decoder
            )
            # if eos_token was found in one sentence, set sentence to finished
            if eos_token_id_tensor is not None:
                unfinished_sequences = unfinished_sequences.mul(
    ...
    def greedy_search_with_self_introspective_decoding(
        self,
        input_ids: torch.LongTensor,
        logits_processor: Optional[LogitsProcessorList] = None,
        stopping_criteria: Optional[StoppingCriteriaList] = None,
        ...
        **model_kwargs,
        ...
        # SID: Get self-introspective decoding parameters
        alpha = contrastive_decoding_config.alpha
        beta = contrastive_decoding_config.beta
        fastv_config = contrastive_decoding_config.fastv_config

        # SID: copy model_kwargs for self-introspective decoding.
        sid_model_kwargs = model_kwargs.copy()
        sid_model_kwargs["fastv_config"] = fastv_config
        ...
        while True:
            ...
            # SID: prepare model inputs (with FastV config)
            sid_model_inputs = self.prepare_inputs_for_generation(input_ids, **sid_model_kwargs)
            
            # SID: forward pass to get next token (use FastV for token pruning)
            sid_outputs = self(
                **sid_model_inputs,
                return_dict=True,
                output_attentions=output_attentions,
            ...
            sid_next_token_logits = sid_outputs.logits[:, -1, :]
            
            # KEY PART OF SELF INTROSPECTIVE DECODING
            # (1) compute adaptive plausibility constraint
            cutoff = torch.log(torch.tensor(beta)).to(input_ids.device) + torch.max(next_token_logits, dim=-1, keepdim=True).values
            # (2) contrast two logits
            diffs = (1 + alpha) * next_token_logits - alpha * sid_next_token_logits
            # (3) apply adaptive plausibility constraint
            next_token_logits = torch.masked_fill(diffs, next_token_logits < cutoff, float("-inf"))
    ...
    def sample_with_visual_contrastive_decoding(
        self,
        input_ids: torch.LongTensor,
        logits_processor: Optional[LogitsProcessorList] = None,
        stopping_criteria: Optional[StoppingCriteriaList] = None,
        ...
        **model_kwargs,
        ...
        vcd_model_kwargs = model_kwargs.copy()
        # VCD: change input_embeds to cd_inputs_embeds
        vcd_model_kwargs["inputs_embeds"] = cd_inputs_embeds
        # auto-regressive generation
        while True:
            if synced_gpus:
                # Under synced_gpus the `forward` call must continue until all gpus complete their sequence.
            ...
            # VCD: update model kwargs for next step
            vcd_model_kwargs = self._update_model_kwargs_for_generation(
                vcd_outputs, vcd_model_kwargs, is_encoder_decoder=self.config.is_encoder_decoder
            )
    ...
    def sample_with_self_introspective_decoding(
        self,
        input_ids: torch.LongTensor,
        logits_processor: Optional[LogitsProcessorList] = None,
        stopping_criteria: Optional[StoppingCriteriaList] = None,
        ...
        **model_kwargs,
        ...
        # copy model_kwargs for self-introspective decoding.
        sid_model_kwargs = model_kwargs.copy()
        sid_model_kwargs["fastv_config"] = fastv_config
